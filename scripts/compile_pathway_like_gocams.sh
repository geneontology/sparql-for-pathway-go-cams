...
rm -f $TARGET_DIR/pathway_like_gocams.tsv $TARGET_DIR/pathway_like_gocams_uniq.tsv.uniq $TARGET_DIR/pathway_like_gocams_filtered_multi_bp.tsv $TARGET_DIR/pathway_like_gocams_filtered_multi_bp_cc.tsv
...
sort $TARGET_DIR/pathway_like_gocams.tmp.tsv | uniq > $TARGET_DIR/pathway_like_gocams_uniq.tsv.uniq
grep model.geneontology.org $TARGET_DIR/multi_connected_bp_gocams.tsv | cut -f1 | grep -v -f /dev/stdin $TARGET_DIR/pathway_like_gocams_uniq.tsv.uniq > $TARGET_DIR/pathway_like_gocams_filtered_multi_bp.tsv
